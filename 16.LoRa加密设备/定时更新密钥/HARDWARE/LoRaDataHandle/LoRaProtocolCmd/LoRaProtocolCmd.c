...
void LoRaCtrlNetLedCmd(u8 *data,u16 datalen)
{
	...
	SX1276TxDataCRC32(data,0x09);			//LoRa发送数据
}
...
void LoRaRequestIDIDCmd(u8 *data,u16 datalen)
{
	...
	data[7] = 'I';
	data[8] = 'D';
	memcpy(&data[9],SelfID_Buf,16);				//本机随机数
	
	crc_value = CalCrc16(data,25,0xFFFF);		//获取CRC校验值
	data[25] = crc_value>>8;					//低字节在前
	data[26] = crc_value;						//高字节在后
	
	SX1276TxDataCRC32(data,27);					//LoRa发送数据
}
...
//请求更新SM4密钥指令应答
void LoRaRequestUpdateSM4KeyACK(u8 *data,u16 datalen)
{
	u16 crc_value = 0;
