/***********************************************************************
	* 文件说明：W25Qxx驱动程序、读/写操作
	* 作者：	ZWQ
	* 时间：	2019-1-21
	* 备注：	芯片信息：	4KB为一个扇区(Sector)；
							16个扇区为1个块(Block)；
...
{
	W25QxxSPI_Init();							//初始化SPI
    SPIX_SetSpeed(SPI_BaudRatePrescaler_4);		//设置为18M时钟,高速模式
   
	W25QXX_TYPE = W25QXX_ReadID();		//读取FLASH ID
	while(W25QXX_TYPE != W25Q32)		//检测芯片能否正常工作
	{
		W25QXX_TYPE = W25QXX_ReadID();	//读取FLASH ID
	#ifdef UART_DUBUG
...
//WriteAddr:开始写入的地址(24bit)
//NumByteToWrite:要写入的字节数(最大65535)
u8 W25QXX_BUFFER[4096];		//定义2KB的数组
void W25QXX_Write(u8* pBuffer, u32 WriteAddr, u16 NumByteToWrite)
{
    ...
    u16 secremain;
    u16 i;
    u8 	*W25QXX_BUF;				//定义字符指针
    W25QXX_BUF = W25QXX_BUFFER;		//指针赋值
