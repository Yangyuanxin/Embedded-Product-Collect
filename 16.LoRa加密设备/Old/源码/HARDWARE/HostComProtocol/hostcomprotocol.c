...
void HostComSoftWareInit(void)
{
	...
	if(hostcom_struct.WhichUart == UART1)
	{
		...
		DMA1_UartSendData((u8 *)"串口1DMA1测试... ...\r\n",strlen("串口DMA1测试... ...\r\n"),hostcom_struct.WhichUart);
	...
	else if(hostcom_struct.WhichUart == UART3)
	{
		...
		DMA1_UartSendData((u8 *)"串口3DMA1测试... ...\r\n",strlen("串口DMA1测试... ...\r\n"),hostcom_struct.WhichUart);
		...
		if(tempCrc == CalcCrc)
		{
			...
			switch( HostStruct->Comx_RxBuf[0] )
			{
				...
				case 0x49:
					...
					{
						SysYear = HostStruct->Comx_RxBuf[3]<<8 | HostStruct->Comx_RxBuf[4];
						SysMonth = HostStruct->Comx_RxBuf[5];
						SysDate = HostStruct->Comx_RxBuf[6];
						SysHour = HostStruct->Comx_RxBuf[7];
