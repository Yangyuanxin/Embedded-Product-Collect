...
_calendar_obj calendar;	//定义时钟结构体
...
u8 RTC_Init(void)
{
    ...
    RCC_APB1PeriphClockCmd(	RCC_APB1Periph_PWR | 			//使能PWR和BKP外设时钟
	...
	if(BKP_ReadBackupRegister(BKP_DR1) != 0x5050)			
    {
        ...
        RTC_WaitForLastTask();						//等待最近一次对RTC寄存器的写操作完成
        RTC_Set(2018, 8, 27, 16, 22, 55);			//设置时间
...
u8 RTC_Set(u16 syear, u8 smon, u8 sday, u8 hour, u8 min, u8 sec)
{
    ...
    {
        if( Is_Leap_Year(t) )
            seccount += 31622400;	//闰年的秒钟数(366*24*60*60 = 31622400)
        else
            seccount += 31536000;	//平年的秒钟数(365*24*60*60 = 31536000)
    }
    smon -= 1;
    ...
    for(t = 0; t < smon; t++)
    {
        seccount += (u32)mon_table[t] * 86400;	//月份秒钟数相加(1天的秒数	--	24*60*60=86400)
